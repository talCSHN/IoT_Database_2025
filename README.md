...
- SQL 기초
    ...
    - DML 중 INSERT, UPDATE, DELETE : [SQL](./day03/db02_dml_query.sql)
        ...
        VALUES (값리스트);  -- VALUE는 SQL표준이 아님(MySQL, PostgreSQL 등에서만 사용 가능)
...
## 7일차
- Workbench Tip
    - SQL툴 공통으로 SELECT 실행 시 모든 행을 다 표시하지 않음(성능저하 방지)
    - Workbench는 1000행으로 제한
    - 성능테스트 시 1000개 제한 풀어줘야함
...
- 데이터모델링 실습
    - 병원업무관리 ERD
        - 요구사항으로 개체와 관계를 정립. 개체에 속하는 속성, 식별자 결정
        - 테이블 명세서 작성(엑셀, 워드)
        - ERWin | Workbench 모델링에서 ERD 작성
        - 생성 스크립트 : [SQL](./day08/db03_병원업무관리_스키마.sql)
        - Workbench에서 DB 생성 후 위 스크립트 실행. DB 구현

    <img src = "./images/db006.png" width="800">

## 9일차
- tkinkter DB연동 GUI앱 개발 : [SQL](./day09/students_regapp.py)
    1. MySQL madang DB 사용하는 madang 사용자 생성, 권한
    2. madang DB에 students 테이블 생성 및 더미데이터 추가
    ```sql
    DROP TABLE IF EXISTS students;
    CREATE TABLE students (
        std_id  INTEGER PRIMARY KEY AUTO_INCREMENT, -- AUTO_INCREMENT는 MySQL에서만 동작하는 옵션
    ...
    <img src="./images/db007.png" width="700">

    4. DB CRUD 함수 구현

    <img src="./images/db008.png" widht="700">    
